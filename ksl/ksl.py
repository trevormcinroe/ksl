...
class KSLAgent:
	...
	def update_critic(self, obs, obs_aug, action, reward, next_obs, next_obs_aug, not_done, logger, step):
		with torch.no_grad():
			...
			# To recover RAD, simply comment out the remainder of this "with" context block & the next comment (down
			# several lines, after the "with" context block)
		...
		# (2nd part of RAD-recovery) Comment out the following two operations: (1) self.critic(obs_aug, action) and
		# (2) += F.mse_loss(...)  + F.mse_loss(...)
